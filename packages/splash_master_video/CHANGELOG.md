...
- [Breaking] - Extracted `VideoSplash` widget from `splash_master` into its own dedicated package.
- [Breaking] - `SplashMaster.video(...)` replaced by `VideoSplash(...)`.
- [Breaking] - `SplashMaster.initialize()` replaced by `VideoSplash.initialize()`.
- [Breaking] - `SplashMaster.resume()` replaced by `VideoSplash.resume()`.
